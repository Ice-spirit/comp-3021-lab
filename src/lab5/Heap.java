...
public class Heap<T extends Comparable<T>> {
    ...
    public T poll(){
        //TODO
    	if (container.size()==0)  throw new IllegalStateException("IllegalStateException!");
    	T Return = container.get(0);
    	container.set(0, container.get(container.size()-1));
    	container.remove(container.size()-1);
    	heapifyDown();
        return Return; // replace this line with implementation
    }
    ...
    private void heapifyUp() {
    	if (container.size()>1) {
	        ...
	        while (hasParent(pos) && container.get(pos).compareTo(container.get(ParentIndex)) < 0) {
