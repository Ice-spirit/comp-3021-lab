...
public class FindMax {
	...
	public void printMaxThread() {
		int three_number_array[] = new int[3];
		// this is a multi threaded version
		Thread t1 = new Thread(() -> {three_number_array[0] = findMax(array, 0, 29);});
		Thread t2 = new Thread(() -> {three_number_array[1] = findMax(array, 30, 59);});
		Thread t3 = new Thread(() -> {three_number_array[2] = findMax(array, 60, 89);});
		t1.start();
		t2.start();
		t3.start();
		try {
		...
		int max = findMax(three_number_array, 0, 2);
		System.out.println("the max value is " + max);
	}
